...
export const Sidebar = ({ activeTab, onTabChange, user, isOpen }) => {
  ...
  return (
    ...
    >
      <div className={`h-full flex flex-col transition-all duration-300 ${isOpen ? 'w-72' : 'w-20'}`}>
        ...
        <div className={`p-4 ${isOpen ? 'flex items-center gap-3' : 'flex justify-center'}`}>
          ...
          {isOpen && (
            <div className="flex flex-col">
              <h1 className="font-bold text-slate-800 text-lg tracking-tight">每日计划</h1>
              <span className="text-[10px] text-slate-400 font-medium">Daily Planner</span>
            </div>
          )}
        </div>
              ...
              <div key={item.id} className="relative group">
                ...
                >
                  ...
                  <div className={`relative p-2 rounded-lg transition-all duration-200 ${
                    ...
                    {/* 图标光晕效果 */}
                    {isActive && (
                      <div className="absolute inset-0 bg-indigo-200 rounded-lg blur-md opacity-50 -z-10" />
                    )}
                  </div>
                  ...
                  <div className={`absolute inset-0 rounded-xl transition-opacity duration-300 ${
                    ...
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-20 h-20 bg-indigo-100 rounded-full blur-xl" />
                  </div>
                </button>
                ...
                {!isOpen && isHovered && (
                  <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 z-50">
                    <div className="bg-slate-800 text-white px-3 py-2 rounded-lg text-sm font-medium shadow-lg whitespace-nowrap">
                      {item.label}
                      <div className="absolute left-0 top-1/2 -translate-x-1 w-2 h-2 bg-slate-800 rotate-45" />
                    </div>
                  </div>
                )}
              </div>
        ...
        {/* 底部用户区域 */}
        <div className={`p-3 border-t border-slate-100 ${isOpen ? '' : 'flex justify-center'}`}>
          <div 
            className={`cursor-pointer rounded-xl p-2 transition-all duration-200 hover:bg-slate-100 ${
          ...
          >
            ...
            {isOpen && (
              <div className="flex-1 min-w-0">
                <div className="font-bold text-slate-700 text-sm truncate">{user.displayName}</div>
